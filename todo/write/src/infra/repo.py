...
class TodoRepository:
    def save(self, todo: Todo, event: TodoCreated, command_id: UUID) -> None:
        with get_db_transaction() as cur:
            # 1. Idempotency Check
            cur.execute(
                "SELECT result_status, result_body FROM processed_commands WHERE command_id = %s",
                (command_id,)
            )
            ...
            cur.execute(
                """
                INSERT INTO todos (id, title, description, priority, due_date, is_completed, created_at, updated_at)
            ...
            cur.execute(
                "INSERT INTO processed_commands (command_id, result_status, result_body) VALUES (%s, %s, %s)",
