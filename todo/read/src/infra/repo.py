...
class TodoReadRepository:
    def list_todos(
        ...
        """
        Query side: Lists todos with pagination, filtering, and sorting directly from the write table.
        """
        offset = (page - 1) * limit
        ...
        query = f"""
            ...
            FROM santiago_munoz_todos
            {where_clause}
            ORDER BY {sort_by} {order}, id ASC
            LIMIT %s OFFSET %s
        """
        
        count_query = f"SELECT COUNT(*) as total FROM santiago_munoz_todos {where_clause}"
